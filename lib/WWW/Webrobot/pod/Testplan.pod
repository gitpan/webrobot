...
Testplan - how to write a test plan for WWW::Webrobot
...
Example:
        ...
        <include file='a file name'>
            ...
            <parm name='useragent' value='koester'/>
        </include>

=head1 Tag <cookies>

By default cookies are on.
You can switch cookies off or you can just clear the cookies.
...
This is done with the data tag:
        ...
        <data>
            <parm name='login' value='yourself'/>
            <parm name='password' value='secret'/>
        </data>

B<RESTRICTION:> The data tag will work for the POST method only.
For the GET method you can and have to code it into the url:
...
The class gets the tags child element as input.
...
For more information see L<WWW::Webrobot::pod::Assert.pod>.
...
Figure out your desired xpath expression with L<bin/xpath.pl>
...
The default assertion (if you do not annotate any assertion) is
C<< <status value='2'/> >>.
...
For more information see L<WWW::Webrobot::pod::Recur.pod>.
...
see L<WWW::Webrobot::pod::Config.pod>.
You can use a name in your test plan
by including it in curly braces like C<{myhost}>.
Example:
...
Some strings are converted to iso-latin,
so you might encounter problems if you rely on anything else but iso-latin.
