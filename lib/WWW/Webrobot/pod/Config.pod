...
=head1 SYNOPSIS
 ...
 names=port=4321
 names=application=http://google:4321/troja

 # alternate forms
 auth.basic=,site-authentification,mylogin,secretpassword
 auth.basic=|site-authentification|mylogin|secretpassword
 auth.basic=/site-authentification/mylogin/secretpassword
...
a list: The first character is the delimiter (see C<auth.basic>).

=back

All C<property_name>s that are marked B<@> accept listmode
(see C<auth.basic>, C<proxy>).
...
and C<WWW::Webrobot::Print::Html> for HTML output.

        output=WWW::Webrobot::Print::Test
        output=WWW::Webrobot::Print::Html

See L<WWW::Webrobot::pod::OutputListeners.pod>
...
Emulates a very common bug of popular www browsers concerning http response 302.
...
The C<load.*> properties are evaluated in L<bin/webrobot_load.pl> only.
